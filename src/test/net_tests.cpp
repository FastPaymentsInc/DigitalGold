...
BOOST_AUTO_TEST_CASE(get_local_addr_for_peer_port)
{
    ...
    // Without the fix peer_us:7633 is chosen instead of the proper peer_us:bind_port.
    ...
    // Without the fix peer_us:7633 is chosen instead of the proper peer_us:peer_us.GetPort().
...
BOOST_AUTO_TEST_CASE(advertise_local_address)
{
    ...
    CAddress addr_ipv4{Lookup("1.2.3.4", 7633, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv4.IsValid());
    BOOST_REQUIRE(addr_ipv4.IsIPv4());

    CAddress addr_ipv6{Lookup("1122:3344:5566:7788:9900:aabb:ccdd:eeff", 7633, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv6.IsValid());
    BOOST_REQUIRE(addr_ipv6.IsIPv6());

    CAddress addr_ipv6_tunnel{Lookup("2002:3344:5566:7788:9900:aabb:ccdd:eeff", 7633, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv6_tunnel.IsValid());
    BOOST_REQUIRE(addr_ipv6_tunnel.IsIPv6());
    BOOST_REQUIRE(addr_ipv6_tunnel.IsRFC3964());

    CAddress addr_teredo{Lookup("2001:0000:5566:7788:9900:aabb:ccdd:eeff", 7633, false).value(), NODE_NONE};
    ...
    CService service_cjdns{Lookup("fc00:3344:5566:7788:9900:aabb:ccdd:eeff", 7633, false).value(), NODE_NONE};
