...
Below are some notes on how to build DigitalGold for Windows.

The options known to work for building DigitalGold on Windows are:
...
Note that for WSL the DigitalGold source path MUST be somewhere in the default mount file system, for
example /usr/src/digitalgold, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\digitalgold`, for example:

    make install DESTDIR=/mnt/c/workspace/digitalgold
