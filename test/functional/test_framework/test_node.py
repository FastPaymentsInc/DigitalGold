...
"""Class for digitalgold node under test"""
...
class TestNode():
    """A class for representing a digitalgold node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        ...
        self.bitcoinconf = self.datadir_path / "digitalgold.conf"
        ...
        # Configuration for logging is set as command-line args rather than in the digitalgold.conf file.
        # This means that starting a digitalgold using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
    ...
    def __del__(self):
        # Ensure that we don't leave any digitalgold processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time digitalgold is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by digitalgold, and
        ...
        self.running = True
        self.log.debug("digitalgold started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the digitalgold process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'digitalgold exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; digitalgold is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to digitalgold after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; digitalgold is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into digitalgold")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to digitalgold
        expected_msg: regex that stderr should match when digitalgold fails

        Will throw if digitalgold starts without an error.
        Will throw if an expected_msg is provided and it does not match digitalgold's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'digitalgold exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'digitalgold should have exited within {self.rpc_timeout}s '
